...
\begin{equation*}
    \frac{\partial ^2 I}{\partial y^2}(x,y) =\frac{1}{k^2}\left( I(x,y+k) + I(x,y-k) - 2\times I(x,y)\right)
\end{equation*}
...
Cette matrice possède donc beaucoup de zéros. Il est donc plus avantageux de la stocker sous cette forme, c'est à dire en ne gardant en mémoire que les coefficients non-nuls et leurs positions dans la matrice.Cela évite de stocker en mémoire une matrice entière.\\
...
\end{pmatrix}
\end{equation}
La solution de ce système existe bien. En effet, A étant carrée, de taille $\left(M\times N, M \times N\right)$. C'est une matrice pentadiagonale, non symétrique. Ses colonnes étant linéairement indépendantes, elle est donc inversible.\\
